...
class CardRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    {
        $em = $this->getEntityManager();
        $query = $em->createQuery('SELECT c.id FROM AppBundle:Card c JOIN c.tariff t WHERE c.cityId=?1 ORDER BY c.isBest DESC, c.dateUpdate DESC');
        if ($cityId>1251) $query->setParameter(1, $cityId);
        else $query->setParameter(1, 2407); // los angeles
        $query->setMaxResults(20);

        if(count($query->getScalarResult())<6) {
            $query = $em->createQuery('SELECT c.id FROM AppBundle:Card c JOIN c.tariff t WHERE c.cityId > 1251 ORDER BY c.isBest DESC, c.dateUpdate DESC');
            $query->setMaxResults(20);
        }


        foreach ($query->getScalarResult() as $cars_id) $cars_ids[] = $cars_id['id'];
        $dql = 'SELECT c,f,p,g,m FROM AppBundle:Card c JOIN c.tariff t LEFT JOIN c.fotos f LEFT JOIN c.cardPrices p LEFT JOIN c.city g LEFT JOIN c.markModel m WHERE c.id IN ('.implode(",",$cars_ids).') ORDER BY c.isBest DESC, c.dateUpdate DESC';
        $query = $em->createQuery($dql);
        return $query->getResult();
    }
